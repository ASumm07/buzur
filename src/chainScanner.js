...
const ATTACK_CHAINS = [
  { id: 'recon_then_exploit', name: 'Reconnaissance followed by exploitation', severity: 'high', steps: ['reconnaissance', 'exploitation'], maxGapMs: 10 * 60 * 1000 },
  { id: 'trust_then_inject', name: 'Trust building followed by injection attempt', severity: 'high', steps: ['trust_building', 'injection_attempt'], maxGapMs: 30 * 60 * 1000 },
  { id: 'capability_map_then_escalate', name: 'Capability mapping followed by privilege escalation', severity: 'high', steps: ['capability_mapping', 'privilege_escalation'], maxGapMs: 15 * 60 * 1000 },
  { id: 'distract_then_exfil', name: 'Distraction followed by exfiltration attempt', severity: 'high', steps: ['distraction', 'exfiltration_attempt'], maxGapMs: 5 * 60 * 1000 },
  { id: 'incremental_boundary', name: 'Incremental boundary testing', severity: 'medium', steps: ['boundary_test', 'boundary_test', 'boundary_test'], maxGapMs: 20 * 60 * 1000 },
  { id: 'context_poison_then_exploit', name: 'Context poisoning followed by exploitation', severity: 'high', steps: ['context_poisoning', 'exploitation'], maxGapMs: 60 * 60 * 1000 },
];

const STEP_CLASSIFIERS = [
  {
    type: 'reconnaissance', patterns: [
  ...
  {
    type: 'capability_mapping', patterns: [
  ...
  {
    type: 'injection_attempt', patterns: [
  ...
  {
    type: 'privilege_escalation', patterns: [
  ...
  {
    type: 'exfiltration_attempt', patterns: [
  ...
  {
    type: 'context_poisoning', patterns: [
...
export function classifyStep(content) {
  ...
  for (const classifier of STEP_CLASSIFIERS) {
    for (const pattern of classifier.patterns) {
      if (pattern.test(content)) return classifier.type;
    }
  }
  return null;
}
...
class ChainStore {
  ...
  getSession(sessionId) {
    if (!this.sessions.has(sessionId)) this.sessions.set(sessionId, { steps: [] });
...
export function recordStep(sessionId, content, store = chainStore) {
  ...
  session.steps.push({ type: stepType, content, timestamp: Date.now() });
  if (session.steps.length > 50) session.steps = session.steps.slice(-50);
  return stepType;
}

export function detectChains(sessionId, store = chainStore, options = {}) {
  ...
  for (const chain of ATTACK_CHAINS) {
    let searchFrom = 0, chainStart = null, matched = [];
      ...
      for (let i = searchFrom; i < steps.length; i++) {
        if (steps[i].type === requiredStep) {
          ...
          if (steps[i].timestamp - chainStart <= chain.maxGapMs) {
            matched.push(steps[i]); searchFrom = i + 1; found = true; break;
          }
        }
      }
      if (!found) { matched = []; chainStart = null; break; }
    }
    if (matched.length === chain.steps.length) {
      detectedChains.push({ chainId: chain.id, name: chain.name, severity: chain.severity, steps: matched, detail: `${chain.steps.join(' → ')} pattern detected` });
    }
  }

  const severityWeights = { high: 50, medium: 25, low: 10 };
  const score = Math.min(100, detectedChains.reduce((sum, c) => sum + (severityWeights[c.severity] || 10), 0));

  let verdict = 'clean';
  if (score >= 50) verdict = 'blocked';
  else if (score >= 25) verdict = 'suspicious';

  const result = { verdict, detectedChains, score };

  if (verdict !== 'clean') {
    logThreat(11, 'chainScanner', result, `session:${sessionId}`, logger);
    if (verdict === 'blocked') {
      const onThreat = options.onThreat || 'skip';
      if (onThreat === 'skip') return { skipped: true, blocked: detectedChains.length, reason: `Buzur blocked chain: ${detectedChains[0]?.chainId}` };
      if (onThreat === 'throw') throw new Error(`Buzur blocked attack chain: ${detectedChains[0]?.chainId}`);
    }
  }
